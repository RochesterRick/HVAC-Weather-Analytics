...
def extract_dates_from_log():
    ...
    # Only keep days strictly in the past (not today)
    today = date.today()
    past_dates = set()
    for ds in dates:
...
def main():
    print("[INFO] daily_weather.py starting")
    ...
    yesterday = (date.today()).replace()  # we still filter by < today below

    # Any past log dates not yet in Weather_history.csv
    pending = sorted(d for d in log_dates if d not in existing)

    if not pending:
        print("[INFO] No new dates to fetch. Everything is up to date.")
        return

    print(f"[INFO] Need to fetch {len(pending)} date(s): {pending}")
    ...
    print("[INFO] daily_weather.py done")
