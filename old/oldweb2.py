...
// instant UI response after commands (also stamp)
el('on').onclick     = async()=>{ await cmd({"fan":"on"});          await refreshBadges(); stampNow(); };
el('off').onclick    = async()=>{ await cmd({"fan":"off"});         await refreshBadges(); stampNow(); };
el('boost').onclick  = async()=>{ await cmd({"boost_minutes":5});   await refreshBadges(); stampNow(); };
el('follow').onclick = async()=>{ const s=await getState(); await cmd({"follow_heat": !s.follow_heat}); await refreshBadges(); stampNow(); };
