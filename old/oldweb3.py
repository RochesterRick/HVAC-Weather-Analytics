...
// MODE buttons
el('btn-auto').onclick   = async()=>{ await cmd({"follow_heat": true, "cancel_boost": true}); await refreshBadges(); stampNow(); };
el('btn-manual').onclick = async()=>{ await cmd({"follow_heat": false});                    await refreshBadges(); stampNow(); };

// Manual controls (only enabled in Manual)
el('on').onclick     = async()=>{ if(el('on').disabled) return;    await cmd({"fan":"on"});          await refreshBadges(); stampNow(); };
el('off').onclick    = async()=>{ if(el('off').disabled) return;   await cmd({"fan":"off"});         await refreshBadges(); stampNow(); };
el('boost').onclick  = async()=>{ if(el('boost').disabled) return; await cmd({"boost_minutes":5});   await refreshBadges(); stampNow(); };
