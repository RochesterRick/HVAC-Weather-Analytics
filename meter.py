...
with open(pjoin("config.json")) as f:
    CFG = json.load(f)
with open(pjoin("rates.json")) as f:
    RATE = json.load(f)

LAT = CFG["location"]["lat"]
LON = CFG["location"]["lon"]
FAST_SEC = CFG["poll"]["fast_sec"]
MINUTE_SEC = CFG["poll"]["minute_sec"]
DEBOUNCE_SEC = CFG["poll"]["debounce_sec"]
CSV_PATH = pjoin(CFG["files"]["csv"])
STATE_PATH = pjoin(CFG["files"]["state"])
CTRL_PATH = pjoin(CFG["files"]["control"])

ADS_C   = CFG["indoor_sensor"]["ads_channel"]
V_SUP   = CFG["indoor_sensor"]["v_supply"]
R_FIXED = CFG["indoor_sensor"]["r_fixed"]
R0      = CFG["indoor_sensor"]["r0"]
BETA    = CFG["indoor_sensor"]["beta"]
T0K     = CFG["indoor_sensor"]["t0_c"] + 273.15
ALPHA   = CFG["indoor_sensor"]["ema_alpha"]

FURNACE_GPIO = CFG["furnace_gpio"]
FAN = CFG["fan"]["tuya"]
FOLLOW_HEAT_DEFAULT = bool(CFG["fan"].get("follow_heat", True))
...
if _have_gpio:
    try:
        ...
        print("[WARN] GPIO init failed:", e)
        furn_btn = None

dev = None
def tuya_dev():
    """Return a persistent tinytuya device instance."""
    global dev
    if not _have_tuya:
        return None
    if dev is None:
...
def read_indoor_c():
    """Return indoor temp in °C from thermistor via ADS1115."""
    global ema_v
    if ch is None:
        return float("nan")
    try:
        v = ch.voltage
        if v is None or v <= 0.0 or v >= V_SUP:
            return float("nan")
        ema_v = v if ema_v is None else (ALPHA * v + (1.0 - ALPHA) * ema_v)
...
FAN_OFF_DELAY_SEC = 150.0    # keep fan running after heat stops (2.5 min)

# --- auto-mode fan-off timer
fan_off_at = None  # epoch seconds; None = no pending auto-off

def fan_set(on):
    """Low-level write to Tuya. Returns True if state matches desired."""
    d = tuya_dev()
    if d is None:
        return False
    try:
        d.set_status(bool(on), DPS_I)
        s = d.status()
        dps = s.get("dps") or {}
        cur = dps.get(DPS_I)
        if cur is None:
...
def fan_ensure(on):
    """Only send a Tuya command when needed and not too frequently."""
    ...
    if _last_fan_desired is not None and desired == _last_fan_desired:
        if (now - _last_fan_set_ts) < MIN_SET_INTERVAL:
            return
    ...
    # Rate limit actual set
    if (now - _last_fan_set_ts) < MIN_SET_INTERVAL:
        return

    if fan_set(desired):
        _last_fan_desired = desired
        _last_fan_set_ts = now
...
def compute_call_on(mode, stable_call):
    ...
    if mode == "cool":
        # same sense line used; treat as AC call
        return bool(stable_call == 1)
    return False
...
def costs_for_minute(mode, furnace_on, ac_on):
    ...
    if furnace_on or ac_on:
        kWh_blower = (BLOWER["watts"] / 1000.0) * (1.0 / 60.0)
    ...
    if mode == "cool" and ac_on:
        if COOL["mode"] == "kw":
            kWh = float(COOL["condenser_kw"]) * (1.0 / 60.0)
        else:
    ...
    while True:
        ...
        if ctl:
            ...
            if "follow_heat" in ctl:
                with lock:
                    state["follow_heat"] = bool(ctl["follow_heat"])
                changed = True
            if changed:
                write_state()
            clear_control()
        ...
        with lock:
            mode = state["mode"]
            follow = state["follow_heat"]
            current_fan_on = state.get("fan_on", False)

        call_on_now = compute_call_on(mode, stable)

        if follow:
            # -------- AUTOMATIC MODE --------
            # Ignore manual fan_cmd in auto
            fan_cmd = None

            if call_on_now:
                # Call is active → fan ON immediately, cancel any pending off timer
                fan_off_at = None
                fan_ensure(True)
            else:
                # No call → if fan is ON, schedule OFF after FAN_OFF_DELAY_SEC
                if current_fan_on:
                    now_t = time.time()
                    if fan_off_at is None:
                        fan_off_at = now_t + FAN_OFF_DELAY_SEC
                    elif now_t >= fan_off_at:
                        fan_ensure(False)
                        fan_off_at = None
                else:
                    # Fan already off, no timer needed
                    fan_off_at = None
        else:
            # -------- MANUAL MODE --------
            # Furnace/AC calls do NOT affect the fan.
            fan_off_at = None
...
def minute_loop():
    ...
    while True:
        ...
        with lock:
            mode = state["mode"]
            f_on = state["furnace_on"]
            ac_on = state["ac_on"]

        ind_f = c_to_f(read_indoor_c())
        out_f = c_to_f(outdoor_c())
        gas_c, elec_c = costs_for_minute(mode, f_on, ac_on)

        with lock:
            state["indoor_f"] = ind_f
            state["outdoor_f"] = out_f
