...
The Raspberry Pi runs a few Python scripts:

- A meter script reads local HVAC/sensor data and writes regular readings to `meter.csv`
- A Flask web app reads the saved data and displays charts/pages in a browser
- Forecast scripts pull weather forecast data and save snapshots over time
- Accuracy scripts compare old forecasts with the weather that actually happened

Most of the project uses plain CSV files for easy inspection, with SQLite used for forecast history.
